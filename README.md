...
src/
  ...
  config/
    cookies.ts                   # Cookie TTL, etc.,
    index.ts                     # Re-export from the folder,
    links.ts                     # External links,
    project.ts                   # Project id, name
  ...
  domain/
    i18n/
      constants.ts               # PROJECT_ID, SUPPORTED_LOCALES, lang cookie name
    ...
    theme/
      constants.ts               # Theme constants (dark/light, default theme)
      schemas.ts                 # Theme schema
...
In `src/domain/i18n/constants.ts`:
...
```ts
import { getMissingTranslationBanner } from '@/domain/i18n/resolve-locale'
